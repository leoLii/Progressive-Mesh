...
struct VertexSplitTarget {
    ...
    vector<CMyHalfEdge*> halfedgesAroundV0;
...
class Simplification {
    ...
    CMyHalfEdge* FindBoundaryEdgeIncidentToVertexInCW(CMyHalfEdge *baseHalfEdge);
...
public:
    ...
    void InitSimplification(CMyMesh *mesh_in);
    bool EdgeCollapse();
    void VertexSplit();
...
void Simplification::InitSimplification(CMyMesh *mesh_in)
{
    mesh = mesh_in;
...
void Simplification::AssignInitialQ()
{
    for(VertexIter vi = mesh->vertices.begin(); vi != mesh->vertices.end(); vi++){
        ...
        CMyHalfEdge *startHalfEdge, *endHalfEdge;
        
        if(vi->isBoundary == false) startHalfEdge = vi->neighborHe;
        else                        startHalfEdge = FindBoundaryEdgeIncidentToVertexInCW(vi->neighborHe);
        
        CMyHalfEdge *hep = startHalfEdge;
        do{
            CumulateQ(vi, hep->face->ormal, -DotProduct(hep->face->ormal, hep->face->halfedge[0].vertex->coord));
        ...
        if(vi->isBoundary){
            ...
            CrossProduct(boundaryVector, startHalfEdge->face->ormal, pseudoNormal);
            ...
            CrossProduct(boundaryVector, endHalfEdge->face->ormal, pseudoNormal);
            Normalize(pseudoNormal);
...
{
    CMyHalfEdge *hepCollapse = ei->halfedge[0];
...
CMyHalfEdge* Simplification::FindBoundaryEdgeIncidentToVertexInCW(CMyHalfEdge *baseHalfEdge)
{
    CMyHalfEdge *hep = baseHalfEdge;
    do{
...
bool Simplification::IsFinWillNotBeCreated(EdgeIter &ei)
{
    ...
    CMyHalfEdge *startHalfEdgeV0, *startHalfEdgeV1;
    ...
    CMyHalfEdge *hepV0 = startHalfEdgeV0;
    do{
        CMyHalfEdge *hepV1 = startHalfEdgeV1;
        do{
...
void Simplification::VertexSplit()
{
    if(vertexSplitTarget.empty() == false){
        ...
        CMyHalfEdge *hepCollapsed = vertexSplitTarget.top().ei->halfedge[0];
        ...
        for(unsigned int i = 0; i < vertexSplitTarget.top().halfedgesAroundV0.size(); i++){
            CMyHalfEdge *hep = vertexSplitTarget.top().halfedgesAroundV0[i];
...
{
    int n_target_faces = mesh->n_faces*pow(0.9, step);
    
    cerr << "step " << step << " " << n_target_faces << endl;
